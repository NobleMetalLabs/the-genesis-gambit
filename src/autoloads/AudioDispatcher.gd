...
func dispatch_audio(audio : StringName) -> AudioStreamPlayer:
	...
	if FileAccess.file_exists("%s/%s.tres" % [sound_path, audio]) or FileAccess.file_exists("%s/%s.tres.import" % [sound_path, audio]):
		new_player.stream = ResourceLoader.load("%s/%s.tres" % [sound_path, audio])
	elif FileAccess.file_exists("%s/%s.wav" % [sound_path, audio]) or FileAccess.file_exists("%s/%s.wav.import" % [sound_path, audio]):
