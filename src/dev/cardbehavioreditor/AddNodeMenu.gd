...
signal create_node(node : CBEditorGraphNode, position : Vector2)
...
func _ready() -> void:
	...
	for category_name : String in nodes.keys():
		...
		for node_name : String in category_items.keys():
			...
			var node : GDScript = category_items[node_name]
...
func handle_dialog_accept() -> void:
	...
	var node : CBEditorGraphNode = tree_to_nodes[node_item]
	create_node.emit(node.new(), Vector2(self.position))
...
func load_nodes(path : String = "res://src/game/card_behavior/") -> Dictionary:
	...
	for file in nodes_dir.get_files():
		var node_resource : GDScript = load("%s/%s" % [path, file])
